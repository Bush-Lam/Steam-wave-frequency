...
public class towerController : MonoBehaviour
{
    ...
    void OnCollisionEnter2D(Collision2D other) {
        ...
        {
            EnemyHealth.transform.Find("EnemyHealthBarDamage").GetComponent<Image>().fillAmount -= Player.GetComponent<ShipController>().bulletPower / 4; // 15 hits
            other.collider.transform.position = new Vector2(300, 300);

            if (EnemyHealth.transform.Find("EnemyHealthBarDamage").GetComponent<Image>().fillAmount <= Player.GetComponent<ShipController>().bulletPower / 4)
            {
